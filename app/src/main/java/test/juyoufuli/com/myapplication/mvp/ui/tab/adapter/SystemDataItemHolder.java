 ...
 * ================================================
 */
public class SystemDataItemHolder extends BaseHolder<SystemBean> {
    ...
    public SystemDataItemHolder(View itemView) {
        super(itemView);
    ...
    @Override
    public void setData(SystemBean data, int position) {
        Observable.just(data.getName())
                .subscribe(s -> mName.setText(s));
        Observable.just(data.getChildren())
                .map(new Function<List<Children>, String>() {
                    @Override
                    public String apply(List<Children> children) throws Exception {
                        StringBuffer name = new StringBuffer();
                        for (int i = 0; i < children.size(); i++) {
                            name.append(children.get(i).getName());
                            name.append("  ");
                        }
